...
class HtmlParser:
    ...
    def parse_main_info(self, html: str) -> dict:
        """This method gets the basic info from the html - header and top columns"""

        print("Getting basic info about the band.")
        soup = BeautifulSoup(html, 'lxml')

        # get basic info from left and right column
        basic_info = self.parse_float_left_right(soup)
    ...
    def parse_float_left_right(self, html: BeautifulSoup) -> dict:
    ...
    def get_releases_links(self, html: str) -> dict:
        """This method gets the links for releases' info from the band's page """
    ...
    def parse_release_data(self, html: str) -> dict:
        """This method gets data from the release page"""

        soup = BeautifulSoup(html, 'lxml')
        release_data = self.parse_float_left_right(soup)
    ...
    def parse_general_comment(self, html:str) -> str:
        """This method gets the general comment for a band"""

        soup = BeautifulSoup(html, 'lxml')
        comment = soup.find('div', class_='displayNone ui-dialog-content ui-widget-content')
        if comment:
            return comment.text
